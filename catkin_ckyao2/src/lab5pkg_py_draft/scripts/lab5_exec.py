...
def main():
    ...
    while len(start_xw_yw_Y) is not 2:
        time.sleep(1)
        start_xw_yw_Y = deepcopy(xw_yw_Y)
    start_xw_yw_G = deepcopy(xw_yw_G)
    while len(start_xw_yw_G) is not 2:
        time.sleep(1)
        start_xw_yw_G = deepcopy(xw_yw_G)
    ...
    for j in range(len(start_xw_yw_G)):
       move_block(pub_command, loop_rate, start_xw_yw_G[j],xw_yw_G_end[j],vel,accel)
...
grid_3x3 = [[0.22, 0.04, 0.035], [0.22, 0.12, 0.035], [0.22, 0.20, 0.035],
            [0.30, 0.04, 0.035], [0.30, 0.12, 0.035], [0.30, 0.20, 0.035],
            [0.38, 0.04, 0.035], [0.38, 0.12, 0.035], [0.38, 0.20, 0.035]]
...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    rospy.loginfo("Moving to start location ...")
    move_arm(pub_cmd,loop_rate,lab_invk(start_xw_yw_zw[0],start_xw_yw_zw[1],0.500,0),4.0,4.0)
    move_arm(pub_cmd,loop_rate,lab_invk(start_xw_yw_zw[0],start_xw_yw_zw[1],0.0318,0),4.0,4.0)
    rospy.loginfo("Gripping ...")
    gripper(pub_cmd,loop_rate,suction_on)
    time.sleep(2.0)
    if digital_in_0 == False:
    ...
    move_arm(pub_cmd,loop_rate,home,4.0,4.0)
    rospy.loginfo("Moving to target location ...")
    move_arm(pub_cmd,loop_rate,lab_invk(target_xw_yw_zw[0],target_xw_yw_zw[1],0.500,0),4.0,4.0)
    move_arm(pub_cmd,loop_rate,lab_invk(target_xw_yw_zw[0],target_xw_yw_zw[1],target_xw_yw_zw[2],0),4.0,4.0)
    rospy.loginfo("Ungripping")
    gripper(pub_cmd,loop_rate,suction_off)
    rospy.loginfo("Moving back to home location ...")
...
def main():
    ...
    while not(isBoardFull(board)):
        ...
        if not(isWinner(board, 'O')):
            p_move = playerMove(start_xw_yw_G, start_xw_yw_Y, board)
            move_block(pub_command, loop_rate, start_xw_yw_G[0],xw_yw_G_end[0],vel,accel)
            printBoard(board)
        else:
        ...
        if not(isWinner(board, 'X')):
            ...
            if move == 0:
                ...
                move_block(pub_command, loop_rate, start_xw_yw_Y[i],grid_3x3[move-1],vel,accel)
